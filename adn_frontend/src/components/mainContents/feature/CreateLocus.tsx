...
const CreateLocus = () => {
  ...
  const handleDelete = async (locusId: string) => {
    ...
    if (confirmation.isConfirmed) {
      try {
        const res = await fetch(`http://localhost:8080/api/locus/delete-locus/${locusId}`, {
          method: 'DELETE',
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
          },
  ...
  const getFormControlClass = (fieldName: keyof Locus) => {
    let className = "w-full px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:border-blue-500";
      ...
      <div className="max-w-full">
        ...
        <div className="bg-[#3F61E9] rounded-lg p-6 mb-6 relative">
          ...
          <div className="bg-green-500 bg-opacity-30 rounded-lg p-2 max-w-xs">
            ...
            <div className="text-blue-100 text-xl">Tổng số locus: {locusList.length}</div>
          </div>
        </div>
        ...
        {showCreateForm && (
          <div className="bg-white border border-gray-200 rounded-lg mb-6 shadow-sm">
            <div className="p-6">
              <h3 className="text-lg font-medium text-gray-800 mb-4">Thêm locus mới</h3>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {locusList.map((locusItem, index) => (
                <tr key={locusItem.locusId || index} className="hover:bg-gray-50">
